                    ...
                    */
                    var newTab = $('<li class="jericho_tabs tab_selected" style="width:0px"  id="jerichotab_' + curIndex + '" dataType="' + ps.data.dataType + '" dataLink="' + ps.data.dataLink + '">' +
                                                    '<div class="tab_left"  style="width:' + (opts.tabWidth - 5) + 'px"  >' +
                                                        ...
                                                        '<div class="tab_text" style="width:' + (opts.tabWidth - 30) + 'px"  >' + ps.title.cut(opts.tabWidth / 10 - 1) + '</div>  ' +
...
function contextmenu(ele) {
    var menu = { width: 150, items: [
        { text: "关闭其他标签", icon: "images/close.png", alias: "contextmenu-close-other", action: contextMenuItem_click },
        { text: "关闭右侧标签", icon: "images/close.png", alias: "contextmenu-close-right", action: contextMenuItem_click }
    ]
    };
    function contextMenuItem_click(target) {
        var cmd = this.data.alias;
        if (cmd == "contextmenu-close-other") {
            var otherTab = $(target).siblings(".jericho_tabs");
            for(var i = 0, k = otherTab.length; i < k; i++){
              $(otherTab[i]).find(".tab_close").children("a").mouseup();  
            }
        }else if (cmd == "contextmenu-close-right") {
            var nextTab = $(target).nextAll();
            for(var i =0, k = nextTab.length; i < k; i++){
              $(nextTab[i]).find(".tab_close").children("a").mouseup();  
            }
        }
    }
    $(ele).contextmenu(menu);
}