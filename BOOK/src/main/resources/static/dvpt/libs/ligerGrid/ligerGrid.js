...
define(['jquery', 'ligerGrid.base', 'ligerTextBox', 'ligerCheckBox', 'ligerComboBox', 'ligerDateEditor'], function($) {
    var l = $.ligerui;
    ...
    $.ligerDefaults.Grid = {
        ...
        iShowScroll: false, // 是否显示滚动条
        iFixColNum: true // 是否固定条数
    };
    ...
    $.ligerui.controls.Grid.ligerExtend($.ligerui.core.UIComponent, {
        ...
        isHorizontalScrollShowed: function() {
            ...
            var inner_table = inner.find('.l-grid-body-table');
            var header_inner = g.gridbody.siblings('.l-grid-header').find('.l-grid-header-inner');
            var header_table = g.gridbody.siblings('.l-grid-header').find('.l-grid-header-table');
            if (!inner.length) return false;
            // 是否显示滚动 TDM 否则改变宽度 
            if (!p.iShowScroll) {
                // console.log(g.gridbody.width());
                inner.css('width', '100%');
                inner_table.css('width', '100%');
                header_inner.css('width', '100%');
                header_table.css('width', '100%');
            }

            // 判断是否存在滚动条 TDM 20160812
            var windowH = $(window).height();
            var contentH = $("#main-content").height();
            g.haScrollBar = contentH > windowH ? true : false;
        ...
        */
        ...
        _updateGridData: function() {
            ...
            // 分页数据条数显示控制 TDM 20160815
            if(p.iFixColNum) {
                // 计算数据实际长度
                if(count < p.pageSize) 
                    for(var i = count; i < p.pageSize; i++) {
                        var obj = data[i - 1];
                        var o = data[i] = {};
                        var pid = previd = obj['__id'];
                        g.formatRecord(o);
                        if (o[p.statusName] == "delete") continue;
                        for(var j in obj) {
                            data[i][j] = '';
        ...
        toggleCol: function(columnparm, visible, toggleByPopup) {
            ...
            $.ligerDefaults.Grid.autoWidth(g);
        },
        //设置列宽
        ...
        _onResize: function() {
            ...
            if (g.enabledFrozen()) {
                ...
                if (gridViewWidth - gridView1Width <= 0) {
                    ...
                    var windowH = $(window).height();
                    var contentH = $("#main-content").height();
                    var haScrollBar = contentH > windowH ? true : false;
                    if(haScrollBar) { // 存在滚动条
                        var changeScroll = (g.haScrollBar == haScrollBar ? false : true);
                        if(changeScroll) {
                            g.gridview2.css({
                                width: gridViewWidth - gridView1Width - 16
                            });
                        } else {
                            g.gridview2.css({
                        ...
                        g.haScrollBar = haScrollBar;
                    } else { // 不存在滚动条
                        var changeScroll = (g.haScrollBar == haScrollBar ? false : true);
                        if(changeScroll) {
                            g.gridview2.css({
                                width: gridViewWidth - gridView1Width + 18
                            });
                        } else {
                            g.gridview2.css({
                                width: gridViewWidth - gridView1Width
        ...
        showFilter: function() {
            ...
            return g.winfilter = $.ligerDialog.open({
                ...
                top: 50,
                buttons: [
                    { text: '确定', onclick: function(item, dialog) { loadData();
                            dialog.hide(); } },
