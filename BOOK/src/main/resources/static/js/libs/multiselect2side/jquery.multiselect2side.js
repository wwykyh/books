	...
	var methods = {
		init : function(options) {
			...
			return this.each(function () {
				...
				var htmlToAdd = 
					"<div class='ms2side__div'>" +
						...
						"<div class='ms2side__options'>" +
							...
							?
							("<p class='AddOne' title='添加选中'>&rsaquo;</p>" +  //todo 更换移动图标
				...
				if(style){
					el.next().find("select").attr("style",style);
					var width=el.next().find("select").width();
					el.next().css("width",(width+2)*2+112); //整个div宽度
					el.next().find(".ms2side__header").css("width",width+2); //提示文本
				}
